...
func TestSwizzlerSetInvalidJSON(t *testing.T) {
	...
	for role, metaBytes := range origMeta {
		newMeta, err := f.MetadataCache.GetMeta(role, store.MaxSize)
		require.NoError(t, err)
...
func TestSwizzlerUpdateSnapshotHashesSpecifiedRoles(t *testing.T) {
	...
	newMeta, err := f.MetadataCache.GetMeta(data.CanonicalSnapshotRole, store.MaxSize)
...
func TestSwizzlerUpdateTimestamp(t *testing.T) {
	...
	newMeta, err := f.MetadataCache.GetMeta(data.CanonicalTimestampRole, store.MaxSize)
	require.NoError(t, err)
