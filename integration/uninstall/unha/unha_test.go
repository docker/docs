...
func (s *TestSuite) TestUninstallNodes() {
	...
	*/
	...
	client, err = utils.GetUserDockerClient(serverURL, utils.GetAdminUser(), utils.GetAdminPassword())
	require.Nil(s.T(), err)
	require.Nil(s.T(), utils.VerifyControllerCount(client, 2, 60))

	// Now proceed to uninstall one other secondary nodes
	client, err = s.ThirdMachine.GetClient()
	require.Nil(s.T(), err)

	log.Infof("Uninstalling third node: %s", id)
	require.Nil(s.T(), utils.RunBootstrapper(client, []string{"uninstall", "-D", "--id", id}, []string{}, []string{}))

	// And finally check cluster has 1 node now
	client, err = utils.GetUserDockerClient(serverURL, utils.GetAdminUser(), utils.GetAdminPassword())
	require.Nil(s.T(), err)
	require.Nil(s.T(), utils.VerifyControllerCount(client, 1, 60))
}
