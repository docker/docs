...
func GetBundle(serverURL, username, password string) (*zip.Reader, error) {
	...
	if err != nil {
		log.Debug("Failed to get bundle")
		return nil, err
	}
...
func GetUserTLSConfig(serverURL, username, password string) (*tls.Config, error) {
	caData := ""
	certData := ""
	keyData := ""
...
func GetUserDockerClient(serverURL, username, password string) (*dockerclient.DockerClient, error) {
	orcaURL, err := neturl.Parse(serverURL)
	if err != nil {
		return nil, err
	}
	...
	// Obtain TLS config
	tlsConfig, err := GetUserTLSConfig(serverURL, username, password)
	if err != nil {
		return nil, err
	}
...
func GetUserEngineAPI(serverURL, username, password string) (*client.Client, error) {
	orcaURL, err := neturl.Parse(serverURL)
	if err != nil {
		return nil, err
	}
	...
	return client.NewClient(orcaURL.String(), version, httpClient, nil)
}
