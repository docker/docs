...
be updated/added. It will be merged with the generated template before being applied on AWS infrastructure.

### Adjusting Load Balancer http HealthCheck configuration

While ECS cluster uses HealthCheck command on container to get service health, Application Load Balancers define
their own URL based HealthCheck mechanism so traffic get routed. As the compose model does not offer such an 
abstraction (yet) the default one will be applied, and will query your service under `/` expecting HTTP status code
`200`.

You can tweak this behaviour using a cloudformation overlay, following [CloudFormation User Guide](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-elasticloadbalancingv2-targetgroup.html) for
...
Application Load Balancer can be used to handle the SSL termination for HTTPS services, so that your code, ran inside
a container, don't have to. This is currently not supported by the ECS integration with lack for an equivalent abstraction in the Compose specification, but you can rely on overlays to enable this feature on generated Listerners
configuration:
