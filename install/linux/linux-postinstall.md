...
> **Note**: Docker EE customers with a UCP subscription can get remote CLI access to UCP through the use of a UCP client
> bundle. A UCP Client Bundle is generated by UCP and secured by mutual TLS. See the document on 
> [CLI access for UCP](https://docs.docker.com/datacenter/ucp/2.2/guides/user/access-ucp/cli-based-access/) for more 
> information. 

> **Warning**: Before configuring Docker to accept connections from remote hosts it is critically important that you
...
Configuring Docker to accept remote connections can be done using the docker.service systemd unit file for Linux distributions using systemd such as recent versions of RedHat, CentOS, Ubuntu and SLES, or via the daemon.json file which is recommended for Linux distributions that do not use systemd.

> **Warning**: Configuring docker to listen for connections using both the systemd unit file and the daemon.json 
> file causes a conflict that prevents docker from starting.

1. Configuring remote access via systemd unit file:
    ...
    5.  Check to see whether the change was honored by checking the output of netstat to confirm dockerd is 
        listing on the configured port.
...
2. Configuring remote access via daemon.json for Linux distributions without systemd:
    ...
    3. Check to see whether the change was honored by checking the output of netstat to confirm dockerd is 
       listing on the configured port.
