...
## Synopsis
If a manifest is pulled by digest from a registry 2.3 with Docker Engine 1.9
...
serves Schema 1 manifests. Engine 1.10 first
attempts to send a Schema 2 manifest, falling back to sending a
...
The Engine constructs a Schema 2 manifest which the
...
manifest is converted on-the-fly to Schema 1 and sent in the
response. The Docker Engine 1.9 is compatible with this older format.

When the manifest is pulled by digest with Docker Engine 1.9 and older, the
...
For this reason if a manifest is pulled by digest from a registry 2.3 with Docker
...
The Docker Engine constructs a Schema 1 manifest which the
registry persists to disk.

When the manifest is pulled by digest or tag with any Docker version, a
Schema 1 manifest is returned.