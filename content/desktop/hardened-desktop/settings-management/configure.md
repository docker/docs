...
| `containersProxy` (Beta) | | Allows you to create air-gapped containers where all container traffic is filtered by the proxy set in the `.pac` file. If `mode` is set to `manual`.... The `transparentPorts` property...|
