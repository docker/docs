...
params:
  image: /assets/images/learning-paths/docker-build-cloud.webp
  skill: Beginner
  time: 10 minutes
  prereq: None
---
...
- Anyone that wants to tackle common causes of slow image builds: limited local
  resources, slow emulation, and lack of build collaboration across a team.
- Developers working on older machines that want to build faster.
- Development teams working on the same repository that want to cut wait times
  with a shared cache.
- Developers performing multi-architecture builds that don’t want to spend
  hours configuring and rebuilding for emulators.
...
that you can use in your workflows. These actions include building and pushing
Docker images, signing in to a Docker registry, initiating a BuildKit builder,
extracting metadata from Git reference and GitHub events, installing QEMU
static binaries for multi-arch builds, enabling high-level builds with Bake,
and analyzing Docker images for security vulnerabilities
...
same. You don't need to create new pipelines.
Learn more in the [CI documentation](/build-cloud/ci/) for Docker Build Cloud.
