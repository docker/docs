...
params:
  image: /assets/images/learning-paths/docker-compose.webp
  skill: Beginner
  time: 5 minutes
  prereq: None
---
...
group service configurations specific to each environment (`dev`, `test`, `staging`).
...
For more information on using profiles, see [Using profiles with Compose](/compose/profiles/).

### In what way can I enforce the database service to start up before the frontend service?
...
on a prepared database (e.g., after migrations), consider [health checks](/compose/compose-file/05-services/#healthcheck)
(available in Docker Compose v2.1 and above). Here, you can configure the
frontend to wait until the database passes its health check before starting.
This ensures the database is not only up but also ready to handle requests.

For more information on setting the startup order of your services,
see [Control startup and shutdown order in Compose](/compose/startup-order/).
...
For more information on building Docker images using Compose, see [Compose Build](/compose/compose-file/build/).
...
The `docker compose up` command creates and start all your services. It's
...
- [Compose File Build Specification](/compose/compose-file/build/)
