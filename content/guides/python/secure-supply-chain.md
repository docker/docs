...
Docker Hardened Images are built to SLSA Build Level 3 and ship with a set of signed attestations, including:

- SBOMs in CycloneDX and SPDX formats, listing every component in the image
- CVE reports identifying known vulnerabilities in the image's components
- VEX statements explaining which CVEs do not apply to the image and why
- Secrets scan results confirming no credentials or keys were accidentally included
- Virus scan results from antivirus checks on image layers
- Test results from automated functional checks run against the image
- SLSA provenance documenting how and where the image was built
- Scout health score summarizing the image's overall security posture
...
For the full list of attestation types and how to verify their signatures with Cosign, see [DHI attestations](/manuals/dhi/core-concepts/attestations.md).
...
Update your `Dockerfile` to reference the digest:

```dockerfile
FROM dhi.io/python:3.12@sha256:abc123... AS builder
```
