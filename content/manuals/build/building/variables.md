...
| [BUILDX_BAKE_GIT_AUTH_HEADER](#buildx_bake_git_auth_header)                 | String            | HTTP authentication scheme for remote Bake files (supports optional `.<hostname>` suffix). |
| [BUILDX_BAKE_GIT_AUTH_TOKEN](#buildx_bake_git_auth_token)                   | String            | HTTP authentication token for remote Bake files (supports optional `.<hostname>` suffix).  |
...
> [!NOTE]
> You can set this variable on a per-host basis by appending a hostname suffix
> to the variable name, such as `BUILDX_BAKE_GIT_AUTH_HEADER.gitlab.com`.
...
> [!NOTE]
> You can set this variable on a per-host basis by appending a hostname suffix
> to the variable name, such as `BUILDX_BAKE_GIT_AUTH_TOKEN.gitlab.com`:
> ```console
> $ export BUILDX_BAKE_GIT_AUTH_TOKEN.gitlab.com=$(cat git-token.txt)
> ```
