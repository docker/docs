...
> the `docker` driver, see [Enable CDI devices](/reference/cli/dockerd.md#enable-cdi-devices)
> documentation.
...
Inspecting the `default` builder will show that the `vendor1.com/device` is
...
```dockerfile
# syntax=docker.io/docker/dockerfile-upstream:1.17-labs
FROM busybox
RUN --device=vendor1.com/device \
  env | grep ^FOO=
```

Since [Dockerfile frontend v1.14.0-labs](../buildkit/dockerfile-release-notes.md#1140-labs),
we can use the [`RUN --device` command](/reference/dockerfile.md#run---device)
to request a device during a build. In this case we set `vendor1.com/device`
device name which will request the first device available in the specification.
In this case it will be `foo`.
...
[+] Building 0.4s (5/5) FINISHED                                                                                                                                                                docker:default
 ...
 => resolve image config for docker-image://docker.io/docker/dockerfile-upstream:1.17-labs                                                                                                                0.1s 
 => CACHED docker-image://docker.io/docker/dockerfile-upstream:1.17-labs@sha256:9187104f31e3a002a8a6a3209ea1f937fb7486c093cbbde1e14b0fa0d7e4f1b5                                                          0.0s
...
#2 resolve image config for docker-image://docker.io/docker/dockerfile-upstream:1.17-labs
#2 DONE 0.1s

#3 docker-image://docker.io/docker/dockerfile-upstream:1.17-labs@sha256:9187104f31e3a002a8a6a3209ea1f937fb7486c093cbbde1e14b0fa0d7e4f1b5
#3 CACHED
...
drivers installed in the kernel. This is similar to using [`--gpus=all` with `docker run`](/reference/cli/docker/container/run.md#gpus)
command.
...
> following image is based on Ubuntu and will install the NVIDIA client
> libraries and generate the CDI specification for your GPU in the container
> builder if a device is requested during a build. This image is temporarily
> hosted on Docker Hub under `crazymax/buildkit:v0.23.2-ubuntu-nvidia`.
