...
Install commands inherit the template image's working directory. The
Docker-provided images for built-in agents set `WORKDIR` to
`/home/agent/workspace`, but this isn't necessarily the primary workspace in a
directly mounted or clone-mode sandbox. Don't rely on the current directory to
locate workspace files. Use absolute paths for bundled assets from
`files/home/`.
