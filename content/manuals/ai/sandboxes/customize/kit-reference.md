...
| `credentialFile.structure`               | Declarative JSON shape rendered as typed values, then JSON-encoded, so output is always well-formed JSON. Preferred over `credentialFile.template`; wins when both are set.                        |
