...
- Ubuntu 24.04 and later enables restricted unprivileged user namespaces by
  default, which prevents unprivileged processes from creating user namespaces
  unless an AppArmor profile is configured to allow programs to use
  unprivileged user namespaces.

  If you install `docker-ce-rootless-extras` using the deb package
  (`apt-get install docker-ce-rootless-extras`), then the AppArmor profile for
  `rootlesskit` is already bundled with the `apparmor` deb package.
{{< /tab >}}
...
[INFO] To remove data, run: `/usr/bin/rootlesskit rm -rf /home/testuser/.local/>
```

Unset environment variables PATH and DOCKER_HOST if you have added them to `~/.>
...
To remove the binaries, remove `docker-ce-rootless-extras` package if you insta>
If you installed Docker with https://get.docker.com/rootless ([Install without >
remove the binary files under `~/bin`:
```console
$ cd ~/bin
$ rm -f containerd containerd-shim containerd-shim-runc-v2 ctr docker docker-in>
```
