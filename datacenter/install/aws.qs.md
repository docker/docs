---
title: Deploy Enteprise Edition on Amazon AWS (using Quickstart)
description: Learn how to deploy Docker Enterprise Edition with one click, using an Amazon AWS CloudFormation template
keywords: docker, datacenter, install, orchestration, management
---

Docker Enterprise Edition for Amazon AWS is an one-click deployment of Docker Enterprise Edition on
AWS with Quickstart. It deploys multiple nodes with Docker Enterprise Engine, and then installs
highly available versions of Universal Control Plane and Docker Trusted Registry.
