...
A global service is a service that runs one copy of the task on every node. There is no
...
Good candidates for global services are monitoring agents, log forwarder
or other types of containers that you want to run on every node in the
swarm.

The diagram below shows a service with three replicas in yellow and a global service
in gray.
