...
* Fix `image pull` producing an `No such image` error if the `--platform` option was used. [moby/moby#45562](https://github.com/moby/moby/pull/45562)
* libnetwork: fix a bug where the embedded DNS server would produce an `SERVFAIL` instead of an `NXDOMAIN` error. [moby/moby#45573](https://github.com/moby/moby/pull/45573)
* containerd-integration: fix images without layers not showing in `docker image ls -a`. [moby/moby#45588](https://github.com/moby/moby/pull/45588)
* API: fix an incorrect "invalid JSON" error being returned by the `POST /commit` endpoint. [moby/moby#45568](https://github.com/moby/moby/pull/45568)
* API: fix the `GET /images/{name:.*}/json` endpoint returning `null` instead of an empty array (`[]`) for `RepoTags` and `RepoDigests`. [moby/moby#45564](https://github.com/moby/moby/pull/45564)
