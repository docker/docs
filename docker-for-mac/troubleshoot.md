...
First locate the `com.docker.diagnose` tool.  If you installed Docker Desktop for Mac in the
...
The diagnostics ID (here B8CF8400-47B3-4068-ADA4-3BBDCE3985D9/20180726143610)
...
Docker Desktop for Mac ignores certificates listed under insecure registries, and
...
* For the `hello-world-nginx` example, Docker Desktop for Mac must be running to get to
  ...
  is connected to the Docker Desktop for Mac Engine (not Engine from
  ...
  distinguishing between the two environments, see [Docker Desktop for Mac vs. Docker
...
* Docker Desktop for Mac uses the `HyperKit` hypervisor
  ...
  `HyperKit` by quitting Docker Desktop for Mac temporarily while you work with
...
* If your system does not have access to an NTP server, then after a hibernate
  the time seen by Docker Desktop for Mac may be considerably out of sync with the
