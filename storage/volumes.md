...
- Launch a new container and mount the volume eg `dbdata` on `/dbdata`
- Mount a local host directory as `/backup`
- Pass a command that tars the contents of the `dbdata` volume to a `backup.tar` file inside our `/backup` directory.

```
$ docker run --rm --mount=type=volume,source=dbdata,target=/dbdata -v $(pwd):/backup ubuntu tar cvf /backup/backup.tar /dbdata
```
