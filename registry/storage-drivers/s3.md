...
| `logs3apirequests`  | no | Whether or not to log each S3 API request with the rest of distribution logs. The default is `false`. |
| `logs3apiresponseheaders`  | no | A mapping of log field names to S3 API response headers; the content of the named response header will be set as the value of the named log field. The default is `{}`. |
