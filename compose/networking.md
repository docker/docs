...
See the [links reference](compose-file/compose-file.md#links) for more information.
...
Overlay networks are always created as `attachable`.  You can optionally set the [`attachable`](compose-file/compose-file.md#attachable) property to `false`.
...
Networks can be configured with static IP addresses by setting the [ipv4_address and/or ipv6_address](compose-file/compose-file.md#ipv4_address-ipv6_address) for each attached network.

Networks can also be given a [custom name](compose-file/compose-file.md#name):
...
If you want your containers to join a pre-existing network, use the [`external` option](compose-file/compose-file.md#external)
```yaml
services:
  # ...
networks:
...
- [Top-level `networks` key](compose-file/compose-file.md#networks-top-level-element)
- [Service-level `networks` key](compose-file/compose-file.md#networks)
