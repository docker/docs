...
- `max_attempts`: How many times to attempt to restart a container before giving
  up (default: never give up). If the restart has not succeeded within the configured 
  `window`, this attempt will not count toward the configured `max_attempts` value. 
  For example, if `max_attempts` is set to '2', and there is no successful restart on 
  the first attempt, more than two restarts may be attempted
