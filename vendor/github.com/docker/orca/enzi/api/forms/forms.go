...
func (form *LDAPSettings) validate(baseFieldName string) (apiErrs []*errors.APIError) {
	...
	if form.ServerURL == "" {
		apiErrs = append(apiErrs, errors.InvalidFormField(fqFieldName(baseFieldName, "serverURL"), "this field can not be left blank"))
	}
	if form.StartTLS && strings.HasPrefix(strings.ToLower(form.ServerURL), "ldaps://") {
		apiErrs = append(apiErrs, errors.InvalidFormField(fqFieldName(baseFieldName, "startTLS"), "cannot be combined with 'ldaps://' server URL"))
	}
	if form.RootCerts != "" {
