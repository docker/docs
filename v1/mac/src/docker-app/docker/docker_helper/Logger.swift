...
class ASLLogger {
    ...
    static private let queue = dispatch_queue_create("com.docker.helper.sync_queue", DISPATCH_QUEUE_SERIAL) // maintain output order in serial queue
...
class Logger {
    ...
    static func log(facility: String = "com.docker.helper", level: ASLLogger.Level, content: String) {
    ...
    static private func terminate(msg: String) {
        if let errorMessage = WizardMessageGeneric(message: "Fatal Error", details: msg, icon: "FatalIcon") {
            let exitBtn = errorMessage.addCloseButton("Exit")
            exitBtn.makeDefault()
