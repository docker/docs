...
/* NOTE must match v1/mac/src/docker-app/docker/docker/vmnetd.swift */
...
// Client -> Server: create symlinks using this container folder
struct install_symlinks2 {
  char container[1024]; /* PATH_MAX on OSX */
};

extern int read_install_symlinks2(int fd, struct install_symlinks2 *sym);
extern int write_install_symlinks2(int fd, struct install_symlinks2 *sym);
