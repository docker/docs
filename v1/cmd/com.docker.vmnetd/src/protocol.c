...
int read_command(int fd, enum command *c) {
  ...
  if ((command != ethernet) && (command != uninstall) && (command != install_symlinks) && (command != uninstall_symlinks) && (command != uninstall_sockets) && (command != bind_ipv4) && (command != install_symlinks2)) {
...
int read_install_symlinks2(int fd, struct install_symlinks2 *sym) {
  if (really_read(fd, (uint8_t*) &sym->container, sizeof(sym->container)) == -1) {
    aslLog(ASL_LEVEL_CRIT, "Failed to read stream from client");
    return -1;
  }
  return 0;
...
int write_install_symlinks2(int fd, struct install_symlinks2 *sym) {
  if (really_write(fd, (uint8_t*) &sym->container, sizeof(sym->container)) == -1){
    aslLog(ASL_LEVEL_ERR, "Failed to write stream to server");
    return -1;
  }
  return 0;
