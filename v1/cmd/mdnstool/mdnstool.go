...
import (
	...
	"github.com/docker/pinata/v1/mdnsmon"
)

func main() {

	hostname := flag.String("hostname", "docker.local.", "Hostname - must be FQDN and end with .")
	instance := flag.String("instance", "Boot2docker", "Instance description")
	port := flag.Int("port", 22, "Service port")
	srv := flag.String("service", "_ssh._tcp", "SRV service type")
	info := flag.String("info", "Boot2docker", "TXT service description")
	ifaceName := flag.String("if", "eth0", "Network interface to bind multicast listener to. This interface will be monitored for IP changes.")
