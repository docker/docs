...
////////////////////////// DOCKER INC CUSTOMIZATION BEGIN //////////////////////
/**
 * @brief Open a custom channel with arbitrary type and payload.
 *
 * @param[in]  channel  An allocated channel.
 * @param[in]  type     The channel type.
 * @param[in]  payload  The channel request payload.
 *
 * @return              SSH_OK on success,
 *                      SSH_ERROR if an error occurred,
 *                      SSH_AGAIN if in nonblocking mode and call has
...
#ifdef WITH_SSH1
#error ssh_channel_open_custom function has not been validated for SSH1 protocol
    if (channel->session->version == 1) {
        return channel_open_session1(channel);
    }
#endif
