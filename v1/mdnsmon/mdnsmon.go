...
// MDNSServer is a MDNS server
type MDNSServer struct {
	service   *mdns.MDNSService
	iface     *net.Interface
	ipUpdates chan []net.IP
	shutdown  chan int
}
...
func NewServer(hostname string, instance string, port int, srv string, info []string, iface *net.Interface) (*MDNSServer, error) {
	...
	ipUpdates := make(chan []net.IP)
	shutdown := make(chan int)

	return &MDNSServer{service: service, ipUpdates: ipUpdates, shutdown: shutdown, iface: iface}, nil
}
...
func (m *MDNSServer) runServer() {
	...
	for {
		select {
		case ips := <-m.ipUpdates: // New IP set received, registering service
