// api-docs prepares a source-preserving reference model for the opt-in prototype.
package main
...
func (d *Document) validate(metaDir string) {
	...
	if len(obj(d.Root["webhooks"])) > 0 {
		d.issue("capability", "/webhooks", "Webhook navigation is outside this prototype", false)
	}
	walk(d.Root, "", func(n Object, p string) {
		if len(obj(n["callbacks"])) > 0 {
			d.issue("capability", p+"/callbacks", "Callback navigation is outside this prototype", false)
		}
	})
...
func run() error {
	if len(os.Args) < 3 {
		return errors.New("usage: api-docs check|generate|inspect ROOT [--preview]")
	}
	command := os.Args[1]
	root, _ := filepath.Abs(os.Args[2])
	dir := filepath.Join(root, "prototypes/api-docs")
	...
	preview := len(os.Args) > 3 && os.Args[3] == "--preview"
	exceptions, _ := readJSON(filepath.Join(dir, "exceptions.json"))
	reports := []any{}
	models := []any{}
	blocking := 0
	for _, src := range catalog.APIs {
		d, e := loadDocument(filepath.Join(dir, "converted", src.ID+".yaml"), meta)
		if e != nil {
	...
	out := filepath.Join(root, "tmp/api-prototype")
	...
	if blocking > 0 {
		return fmt.Errorf("%d blocking diagnostics; see tmp/api-prototype/validation.json", blocking)
	}
	if command == "generate" {
		if e = writeJSON(filepath.Join(out, "data/api-prototype.json"), Object{"modelVersion": 1, "apis": models}); e != nil {
			return e
		}
		if e = publishSources(root, catalog.APIs); e != nil {
			return e
		}
	}
	return nil
