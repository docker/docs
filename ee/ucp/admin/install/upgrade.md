...
> **Note:** If you are upgrading a cluster to UCP 3.0.2 or higher on Microsoft Azure, then please ensure all of the Azure [prerequisites](install-on-azure.md/#azure-prerequisites) are met.
...
- Operating system:
  - If cluster nodes OS branch is older (Ubuntu 14.x, RHEL 7.3, and so on), consider patching all relevant packages to the most recent (including kernel).
  - Perform a rolling restart of each node before upgrade (to confirm in-memory settings are the same as startup-scripts).
  - Run `check-config.sh` on each cluster node (after rolling restart) for any kernel compatibility issues.

- Procedural:
  - Back up Swarm, UCP, and DTR before upgrading.
  - Gather compose file/service/stack files.
  - Generate a UCP Support dump (for point in time) pre-upgrade.
  - Load troubleshooting packages (netshoot and other packages).
  - The best order for upgrades is Engine, UCP, and then DTR. Note that the scope of this topic is limited to upgrade instructions for UCP.
  - Preload Engine/UCP/DTR images. If your cluster is offline (with no connection to the internet), then Docker provides tarballs containing all of the required container images [here](/ee/ucp/admin/install/upgrade-offline/). If your cluster is online, you can pull the required container images onto your nodes using the following command:
...
- Upgrade strategy:
  ...
  - Manual cluster upgrade: Performed using the CLI or the UCP UI. Automatically upgrades manager
    ...
    - [Upgrade existing nodes in place](#upgrade-existing-nodes-in-place): Performed using the CLI. Automatically upgrades manager nodes and allows you to control the order of worker node upgrades.
...
[Create a backup](/ee/ucp/admin/backup/) of your cluster. This allows you to recover if something goes wrong during the upgrade process.

> **Note:** The backup archive is version-specific, so you can't use it during the upgrade process. For example, if you create a backup archive for a UCP 2.2 cluster, you can't use the archive file after you upgrade to UCP 3.0.
...
3. Make sure the node is healthy. In your browser, navigate to **Nodes** in the UCP web interface, and check that the node is healthy and is part of the cluster.

## Upgrade UCP

When upgrading UCP to version {{ page.ucp_version }}, you can choose from different upgrade workflows:
...
![Upgrade banner](../../images/upgrade-ucp-1.png){: .with-border}

Clicking this message takes an admin user directly to the upgrade process. The upgrade process can also be found under the **Upgrade** tab of the **Admin Settings** section.

![ Upgrade tab of the Admin Settings section](../../images/upgrade-ucp-2.png){: .with-border}

In the **Available Versions** drop-down list, select the version you want to update.
...
node, giving the user more control to migrate workloads and control traffic
...
> **Note:** The upgrade command should not be run on a workstation with a client bundle.
...
nodes. UCP will constantly monitor this label, and if that label is removed, UCP then upgrades the node.

To trigger the upgrade on a worker node, you need to remove the label.
...
Use this workflow to create a parallel environment for a new deployment, which can greatly reduce downtime, upgrades
...
1. Upgrade manager nodes. The `--manual-worker-upgrade` command automatically upgrades manager nodes first, and then allows you to control the upgrade of the UCP components on the worker nodes using node labels.
...
3. Join the Enterprise Engine to the cluster.

    ```bash
    docker swarm join --token SWMTKN-<YOUR TOKEN> <manager ip>:2377
    ```

4. Pause all existing worker nodes to ensure that new workloads are not deployed on existing nodes.

    ```bash
    docker node update --availability pause <node name>
    ```

5. Drain paused nodes for workload migration. Redeploy workloads on all existing nodes to new nodes. Because all existing nodes are “paused”, workloads are
...
6. Remove drained nodes.
    ...
    - After each node is fully drained, it can be shut down and removed from the cluster. On each worker node being removed from the cluster, run a command similar to the following example:

    ```bash
    docker swarm leave <node name>
    ```

    - Run a command similar to the following example on the manager node when the old worker becomes unresponsive:
...
UCP 3.x.x requires additional opened ports for Kubernetes use. For ports that are used by the latest UCP versions and the scope of port use, refer to
[ports used](https://docs.docker.com/ee/ucp/admin/install/system-requirements/#ports-used).

If you have upgraded from UCP 2.2.x to 3.0.x, verify that the ports 179, 6443, 6444, and 10250 are open for Kubernetes traffic. If you have upgraded to UCP 3.1.x, in addition to the ports listed above, also open ports 9099 and 12388.
...
If you’re running a UCP version earlier than 2.1, first upgrade to the latest 2.1 version, then upgrade to 2.2. Use the following rules for your upgrade path
to UCP 2.2:
