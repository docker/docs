...
- All UCP worker and manager nodes need to be deployed into the same Azure
...
- The Azure Virtual Machine Object Name needs to match the Azure Virtual Machine
  ...
  the FQDN (Fully Qualified Domain Name) of the host including domain names. 
- An Azure Service Principal is required with `Contributor` access to the Azure Resource
  Group hosting the UCP Nodes. If using a separate networking Resource
...
> Note: The `pod-cidr` range must match the Azure Vnet Subnet attached the hosts
