...
| `com.docker.lb.algorithm` | Pick the load balancing algorithm by setting `round_robin`, `ip_hash`, or `least_conn`. With `round_robin`, requests to the application servers are distributed in a round-robin fashion. With `least_conn`, the next request is assigned to the server with the least number of active connections. With `ip_hash`, a hash-function is used to determine what server should be selected for the next request (based on the client’s IP address). If no label is set, `round_robin` will be used to decide the backend server. | `round_robin` or `ip_hash` or `least_conn`  | 
