...
// GetOpenIDConfig retrieves the current System OpenID Configuration. If
// there is an API error response then the returned error will be of the type
// *(github.com/docker/orca/enzi/api/errors).APIErrors
func (s *Session) GetOpenIDConfig() (*responses.OpenIDConfig, error) {
	endpoint := s.buildURL("/v0/config/openid", nil)

	var config responses.OpenIDConfig
	if err := s.performRequest("GET", endpoint, nil, http.StatusOK, &config, nil); err != nil {
		return nil, err
	}
...
// SetAccountsConfig submits a form to set or replace the current System
// Accounts Configuration. If there is an API error response then the returned
// error will be of the type
// *(github.com/docker/orca/enzi/api/errors).APIErrors
func (s *Session) SetAccountsConfig(form forms.OpenIDConfig) (*responses.OpenIDConfig, error) {
	endpoint := s.buildURL("/v0/config/openid", nil)
