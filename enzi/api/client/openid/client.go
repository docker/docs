...
// GetTokenWithAccountName requests a token using the given accountName as a
// refresh token. The use of the account name as a refresh token is valid as
// long as the client application still has authorization by the account or is
// implicitly authorized (privileged). If there is an oauth error response then
// the returned error will be of the type
// *(github.com/docker/orca/enzi/api/client/openid/oautherrors).ErrorResponse
func (client *Client) GetTokenWithAccountName(accountName string) (*TokenResponse, error) {
	// Prepare a token request using the "refresh_token" grant type.
	params := url.Values{}
	params.Set("grant_type", "refresh_token")
	params.Set("refresh_token", accountName)

	return client.requestToken(params)
}

// GetTokenWithAccountID requests a token using the given accountID as a
// refresh token. The use of the account ID as a refresh token is valid as long
// as the client application still has authorization by the account or is
// implicitly authorized (privileged). If there is an oauth error response then
// the returned error will be of the type
// *(github.com/docker/orca/enzi/api/client/openid/oautherrors).ErrorResponse
func (client *Client) GetTokenWithAccountID(accountID string) (*TokenResponse, error) {
	// Prepare a token request using the "refresh_token" grant type.
	params := url.Values{}
	params.Set("grant_type", "refresh_token")
	params.Set("refresh_token", "id:"+accountID)

	return client.requestToken(params)
}

// GetTokenForServiceAudience requests a token using the given accountID as a
// refresh token with the given serviceID as an audience of the token. The use
// of the account ID as a refresh token is valid as long as the client
// application still has authorization by the account or is implicitly
// authorized (priviliged). Another service can be used as an audience of the
// token as long as the account has authorized the client application to access
// that service on its behalf or is implicitly authorized (priviliged). If
// there is an oauth error response then the returned error will be of the type
// *(github.com/docker/orca/enzi/api/client/openid/oautherrors).ErrorResponse
func (client *Client) GetTokenForServiceAudience(accountID, serviceID string) (*TokenResponse, error) {
	// Prepare a token request using the "refresh_token" grant type.
	params := url.Values{}
	params.Set("grant_type", "refresh_token")
	params.Set("refresh_token", "id:"+accountID)
	params.Set("audience", serviceID)
