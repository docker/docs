...
func resolveURL(hostURLString string) (*url.URL, error) {
	...
	if err != nil {
		return nil, apierrors.WithStackTrace(err)
	}
...
func getTLSConfig(settings *config.Settings, serverName string) (*tls.Config, error) {
	config := &tls.Config{
...
func GetConn(hostURLString string, settings *config.Settings) (ldapConn *ldap.Conn, err error) {
	...
	case "ldap":
		...
		if settings.StartTLS {
			...
			if err := ldapConn.StartTLS(tlsConfig); err != nil {
				return nil, apierrors.WithStackTrace(fmt.Errorf("unable to perform StartTLS: %s", err))
			}
		}
	...
	case "ldaps":
		if settings.StartTLS {
			return nil, apierrors.WithStackTrace(fmt.Errorf("cannot use StartTLS with ldaps://"))
		}
		...
		return ldapConn, nil
	default:
		return nil, apierrors.WithStackTrace(fmt.Errorf("%s is an invalid scheme for a LDAP url", hostURL.Scheme))
	}
}
