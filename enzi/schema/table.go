...
func configureDB(session *rethink.Session, replicaCount uint, emergencyRepair bool) error {
	log.Debugf("(Re)configuring Database - replicaCount=%d", replicaCount)
	...
	log.Debugf("(%02d/%02d) Configuring Tables...", numDone, len(allTables))
	...
	for numDone < len(allTables) {
		...
		log.Debugf("(%02d/%02d) Configured Table %q", numDone, len(allTables), result.tableName)
	}
...
func DrainServer(session *rethink.Session, host string, port uint) error {
	...
	log.Debug("Determining Number of Servers in DB Cluster")
	numberOfServers, err := getNumberOfServers(session)
	if err != nil {
		return fmt.Errorf("unable to get number of servers: %s", err)
	}

	log.Debugf("Nodes in DB Cluster: %d", numberOfServers)
