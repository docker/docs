...
func (graph *Graph) setV1CompatibilityConfig(root string, data []byte) error {
	return ioutil.WriteFile(filepath.Join(root, v1CompatibilityFileName), data, 0600)
}

func (graph *Graph) getV1CompatibilityConfig(root string) ([]byte, error) {
	return ioutil.ReadFile(filepath.Join(root, v1CompatibilityFileName))
}
...
// all of it's parents based on the image config JSON.
func (graph *Graph) GenerateV1CompatibilityChain(id string) ([]byte, error) {
	if v1config, err := graph.getV1CompatibilityConfig(graph.ImageRoot(id)); err == nil {
		return v1config, nil
	}
	...
	if err := graph.setV1CompatibilityConfig(graph.ImageRoot(id), json); err != nil {
		return nil, err
	}
	return json, nil
}
