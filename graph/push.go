...
func (s *TagStore) getV1ID(id string) (string, error) {
	jsonData, err := s.getV1Config(id)
	if err != nil {
		return "", err
	}
...
func (s *TagStore) getV1Config(id string) ([]byte, error) {
	jsonData, err := s.graph.GenerateV1CompatibilityChain(id)
	if err != nil {
		return nil, err
	}
	return jsonData, nil
